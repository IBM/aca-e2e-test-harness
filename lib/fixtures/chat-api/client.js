...
class ChatApiClient extends EventEmitter {
  ...
  async init(metadata) {
    return new Promise(async resolve => {
      ...
      let convId = await botChatAPI.postMessage(this.url, this.id, '', transformedMetadata);
      resolve(convId);

      let message = null;
      while (this.poll) {
        await botChatAPI.sleep(100);
        message = await botChatAPI.getMessage(this.url, this.id);
        if (message) {
  ...
  send(message, context, currentTime) {
    ...
    botChatAPI.postMessage(this.url, this.id, message, transformedMetadata);
